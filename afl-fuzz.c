...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  u8  fault = 0, new_bits = 0, var_detected = 0, hnb = 0, hnb_2 = 0,
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (!(hnb = has_new_bits_asm(trace_bits, virgin_bits, &bitmap_changed))) {
    // if (!(hnb = has_new_bits_simd(trace_bits, virgin_bits, &bitmap_changed))) {
...
void profile_function() {
    ...
    // u8* virgin_bits_2 = (u8*)aligned_alloc(32, MAP_SIZE); 
    u8* virgin_bits_2 = (u8*)malloc(MAP_SIZE); 
    if (!virgin_bits_2) {
      perror("Failed to allocate memory for virgin_bits_2");
      exit(1);
    }

    // u8* trace_bits_2  = (u8*)aligned_alloc(32, MAP_SIZE); 
    u8* trace_bits_2  = (u8*)malloc(MAP_SIZE); 
    if (!trace_bits_2) {
      perror("Failed to allocate memory for trace_bits_2");
      exit(1);
    }
    ...
    printf("\nhnb_ok            = %d", hnb_ok);
    printf("\nbitmap_changed_ok = %d", bitmap_changed_ok);
    printf("\n");
    printf("\nhnb               = %d", hnb);
    printf("\nhnb_2             = %d", hnb_2);
...
//     u64* current = (u64*)data->bitmap_start;
//     u64* virgin = (u64*)virgin_bits + data->offset * data->segment_size;

//     int i = data->segment_size >> 3;

//     u8 local_ret = 0;

//     while (i--) {
//         if (unlikely(*current) && unlikely(*current & *virgin)) {
//             if (likely(local_ret < 2)) {
//                 u8* cur = (u8*)current;
//                 u8* vir = (u8*)virgin;
...
//     data->shared_ret[data->offset] = local_ret;
// }

// int has_new_bits_mthread(uint8_t* bitmap, int num_threads, ThreadPool* pool) {
//     uint8_t shared_ret[num_threads];
//     WorkData jobs[num_threads];

//     int segment_size = MAP_SIZE / num_threads;

//     for (int i = 0; i < num_threads; i++) {
//         jobs[i].bitmap_start = bitmap + i * segment_size;
//         jobs[i].segment_size = segment_size;
//         jobs[i].shared_ret = shared_ret;
//         jobs[i].offset = i;
//         threadpool_add_job(pool, thread_has_new_bits, &jobs[i]);
//     }

//     threadpool_wait(pool);

//     uint8_t bitmap_changed = 0;

//     for (int i = 0; i < num_threads; i++) {
//         if (shared_ret[i] > bitmap_changed) {
//             bitmap_changed = shared_ret[i];
//         }
//     }
